...
默认文件数据库设置 `WRITE_DELAY=0`，关闭 H2 的提交写入延迟。发行包测试会在请求完成后终止服务并重启，验证文档和历史恢复；这不等于对硬件故障或断电作保证。自定义数据库 URL 时请保留该设置。参见 [H2 WRITE_DELAY](https://h2database.com/html/commands.html#set_write_delay)。
...
可选的发行包冒烟脚本需要 Python 3.10+，会启动打包后的真实服务，验证导入、检索、回答、重启持久化、删除及历史快照，完成后停止服务。JUnit 模型测试使用本地模拟 HTTP 服务，不需要真实 API Key。
