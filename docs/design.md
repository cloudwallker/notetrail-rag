...
Unicode中文分块边界、重叠、空文本拒绝；真实BM25排序和无命中；非法topK；导入-检索-提问-历史-删除全链路；重开数据库恢复；文档删除后不参与检索而历史快照保留；模拟兼容模型HTTP服务验证headers/body及回答、超时/非2xx/畸形响应错误和secret不泄漏；无命中时零模型调用。
提供静态演示页、启动器、API示例、中文README、设计与学习文档、Windows/Linux CI及可执行JAR。端到端示例无需模型密钥。
